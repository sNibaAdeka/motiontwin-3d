...
export class AvatarScene {
  constructor(container) {
    ...
    this.materials = {
      stick: physicalMaterial(0xdffcff, { emissive: 0x173a42, emissiveIntensity: 0.48, roughness: 0.42 }),
      joint: physicalMaterial(0x43e6c2, { emissive: 0x116b5a, emissiveIntensity: 0.8, roughness: 0.28 }),
      head: physicalMaterial(0xeefeff, { emissive: 0x173a42, emissiveIntensity: 0.32, roughness: 0.48 }),
  ...
  buildAvatar() {
    ...
    for (const segment of SEGMENTS) {
      const mesh = cylinder(segment.top, segment.bottom, this.materials.stick);
    ...
    for (const id of JOINT_IDS) {
      const radius = [23, 24].includes(id) ? 0.043 : 0.034;
    ...
    this.shoulderBar = cylinder(0.026, 0.026, this.materials.stick);
    this.hipBar = cylinder(0.028, 0.028, this.materials.stick);
    this.spine = cylinder(0.03, 0.03, this.materials.stick);
    this.neck = cylinder(0.023, 0.023, this.materials.stick);
    this.neck.name = 'neck';
    this.head = setShadow(new THREE.Mesh(new THREE.SphereGeometry(1, 12, 10), this.materials.head));
    this.head.name = 'head';
    this.leftFoot = cylinder(0.022, 0.022, this.materials.stick);
    this.rightFoot = cylinder(0.022, 0.022, this.materials.stick);
  ...
  updateAvatar(frame) {
    ...
    if (nose) {
      ...
      this.head.scale.setScalar(headRadius * 0.86);
